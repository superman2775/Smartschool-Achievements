...
(function () {
  ...
  console.log("[Achievements] Login Visit Counter gestart");

  const TRACKED_PAGE = { name: 'visitLoginCount', match: '/login' };
  const FLAG = 'wasInsideLogin';
  ...
  function checkVisit() {
    const inside = isLogin();
    const wasInside = sessionStorage.getItem(FLAG) === 'true';

    // Case 1: We enter Login from outside → COUNT
    if (inside && !wasInside) {
    ...
    // Case 2: We left Login → reset flag
    if (!inside && wasInside) {
